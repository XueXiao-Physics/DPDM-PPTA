...
def delay_U1B(toas, pos, log10_ma, log10_eps, normA2_1_e, normA2_2_e, normA2_3_e, normA2_p, phase1_e, phase2_e, phase3_e, phase_p):
    ...
    dx_e2 = - A0 * np.sqrt(normA2_2_e) * eps * e * (1.0 / ma) * bq2m * np.sin(twopif * toas + phase2_e)
    dx_e3 = - A0 * np.sqrt(normA2_3_e) * eps * e * (1.0 / ma) * bq2m * np.sin(twopif * toas + phase3_e)    
    dx_p = - A0 * np.sqrt(normA2_p) * eps * e * (1.0 / ma) * bq2m * np.sin(twopif * toas + phase_p)
...
def delay_U1B_corr(toas, pos, log10_ma, log10_eps, normA2_1_e, normA2_2_e, normA2_3_e, phase1_e, phase2_e, phase3_e, phase_p):
    ...
    dx_p1 = - A0 * np.sqrt(normA2_1_e) * eps * e * (1.0 / ma) * bq2m * np.sin(twopif * toas + phase1_e + phase_p)
    dx_p2 = - A0 * np.sqrt(normA2_2_e) * eps * e * (1.0 / ma) * bq2m * np.sin(twopif * toas + phase2_e + phase_p)
    dx_p3 = - A0 * np.sqrt(normA2_3_e) * eps * e * (1.0 / ma) * bq2m * np.sin(twopif * toas + phase3_e + phase_p)
...
def dpdm_block(model=None,log10_ma=None,log10_eps=None):
    name='x_dp'
